...
//added request Australia didnt use ratecomm for airlines ID.
...
// Updated getCommissionRate function with renamed parameter
const getCommissionRate = (country: string, currency: string, totalFare: number, airlines: string): number => {
  if (country.toUpperCase() === "AUSTRALIA" && airlines === "ID") {
    return 2.0;
  }
...
export default function DashboardPage() {
  ...
  const computeTotals = (): TotalByCurrency[] => {
    ...
    Object.values(totals).forEach((total) => {
      ...
      if (countries.length === 1) {
        total.commissionRate = getCommissionRate(countries[0], total.currency, total.totalFare, filteredData.find(row => row.currency === total.currency)?.airlines || "");
      } else {
        total.commissionRate = 2;
      }
      total.totalCommission = total.totalFare * (total.commissionRate / 100);
  ...
  const exportTableToCSV = () => {
    ...
    const mainValueMappers = [
      ...
      (row: DashboardRow) => {
        const fare = typeof row.totalFare === "number" && !isNaN(row.totalFare) ? row.totalFare : 0;
        const rate = getCommissionRate(row.country, row.currency, fare, row.airlines);
        const commission = fare * (rate / 100);
        return `${commission.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (${rate}%)`;
      },
    ];
