...
//added function for handling 13 digit number
"use client";
...
function CollectionUploader({
    ...
    const currentDate = new Date();
    const maxDate = new Date(currentDate.getFullYear(), currentDate.getMonth() - 2, 1).toISOString().split("T")[0].slice(0, 7);
    currentDate.setMonth(currentDate.getMonth() - 3);
    const minDate = currentDate.toISOString().split("T")[0].slice(0, 7);
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (collectionKey === "dataFmr" && errorResponse.error.includes("TicketNumber")) {
                        newMessages.push(errorResponse.error); // Display specific TicketNumber error
                    } else {
                        newMessages.push(
                            `Failed to upload ${file.name}: ${errorResponse.error || "Unknown error"}`
                        );
    ...
    return (
        ...
        >
            <h4 className="text-sm font-bold mb-2 text-black justify-between flex items-center">
                {collectionName}
                <button
                    onClick={() => setIsExpanded(!isExpanded)}
                    className="text-xs text-black"
                    ...
                    !["mastertbs_45", "mastertbs_46"].includes(collectionKey) ? (
                        <div className="mb-2">
                            ...
                            <input
                                ...
                                value={month}
                                onChange={(e) => setMonth(e.target.value)}
                                className="w-full p-1 rounded bg-primary text-black text-xs border border-black"
                                min={minDate}
                                max={maxDate}
                                required
                            />
